...
for word_instance in word_timeline:
    ...
    if word_frame_list:
        ...
        im = cv.LoadImageM(images_path + capture_date + "/" + word_frame_list[0], cv.CV_LOAD_IMAGE_GRAYSCALE)
        ...
        #im_downsmpl = cv.CreateMat(im.rows/downsmpl_factor, im.cols/downsmpl_factor, cv.CV_8UC1)
        #cv.Resize(im, im_downsmpl) 
        #cv.Canny(im_downsmpl, im_downsmpl, 50, 150)
        #sum = cv.CreateMat(im_downsmpl.rows, im_downsmpl.cols, cv.CV_16UC1)
        #avg = cv.CreateMat(im_downsmpl.rows, im_downsmpl.cols, cv.CV_8UC1)
        sum = cv.CreateMat(im.rows, im.cols, cv.CV_16UC1)
        avg = cv.CreateMat(im.rows, im.cols, cv.CV_8UC1)
        cv.SetZero(sum)
        cv.SetZero(avg)
        ...
        if len(word_frame_list) > 1:
            for frame in word_frame_list[1:]:
                im = cv.LoadImageM(images_path + capture_date + "/" + frame, cv.CV_LOAD_IMAGE_GRAYSCALE)
                im = cv.GetSubRect(im, (205, 145, 225, 95))
                cv.ConvertScale(im,im,-1,255)
                cv.Threshold(im, im, 165, 0, cv.CV_THRESH_TOZERO)
                cv.Canny(im, im, 50, 150)
        ...
        #cv.Canny(avg, avg, 10, 150)
        avg_downsmpl = cv.CreateMat(avg.rows/downsmpl_factor, avg.cols/downsmpl_factor, cv.CV_8UC1)
        cv.Resize(avg, avg_downsmpl)
        avg = avg_downsmpl

        print word_list.index(word),
        for i in range(avg.rows):
            for j in range(avg.cols):
                print str(i*avg.cols+j+1) + ":" + str(avg[i,j]),
